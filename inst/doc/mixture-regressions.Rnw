...
\begin{figure}
  \centering
<<fig=true, echo=false, results=hide>>=
print(plot(betaMixFix_3, nint = 10, mark = 3, col = "grey", layout = c(3, 1)))
@ 
...
Figure~\ref{fig:default-2} shows that component 3 has a slight overlap
...
components was separated from the fit and is now captured as an
expression \citep[to allow for lexical
scoping;][]{mixtures:Gentleman+Ihaka:2000} in the slot
