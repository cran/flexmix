...
library("flexmix")
@ 
\subtitle{\pkg{flexmix} version \Sexpr{packageDescription("flexmix", fields="Version")}\\\today}
\author{Bettina Gr{\"u}n\\
  Technische Universit{\"a}t Wien \And
  Friedrich Leisch\\
...
\Address{
  ...
  1040 Wien, Austria\\
  E-mail: \email{Bettina.Gruen@tuwien.ac.at}
  ...
  E-mail: \email{Friedrich.Leisch@stat.uni-muenchen.de}\\
...
%%-------------------------------------------------------------------------
\begin{document}
\SweaveOpts{engine=R, echo=true, height=5, width=8, eps=FALSE}
...
e.g.~\pkg{mclust} \citep{mixtures:Fraley+Raftery:2002a} for finite
mixtures of Gaussian distributions.

\cite{mixtures:Leisch:2004} gives a general introduction into the
...
regression models is given in \cite{mixtures:Gruen+Leisch:2006}. This
...
described in detail in \cite{mixtures:Leisch:2004}. It takes as
...
\pkg{MASS} \citep{mixtures:Venables+Ripley:2002} with the independent
...
\cite{mixtures:Gruen+Leisch:2006}.  The data is generated from a
...
is a categorical variable with two levels, with nested effects and
another independent variable $x2$, which is a numeric variable, with
fixed effects. The prior probabilities depend on a concomitant
...
repeatedly using \code{stepFlexmix()}.
<<NregFix>>=
library(flexmix)
data(NregFix)
...
coefficients of $x2$. For $x1$ the coefficient of the third component
is not significantly different form zero and the confidence intervals
...
Model2 <- FLXMRglmfix(fixed = ~x2,
                   ...
                   varFix = TRUE)
fittedModel2 <- flexmix(y ~ 1, model = Model2, cluster = posterior(fittedModel),
                        data = NregFix, concomitant = FLXPmultinom(~ w))             
BIC(fittedModel)
BIC(fittedModel2)
@ 
...
\cite{mixtures:McLachlan+Peel:2000} on page 165. The dataset is from a
...
<<beta-glm>>=
data(betablocker)
betaGlm <- glm(cbind(Deaths, Total - Deaths) ~ Treatment, family = "binomial", data = betablocker)
betaGlm
@ 
...
clusters <- factor(cluster(betaMixFix_3), labels = paste("Cluster", 1:3))
...
print(plot(betaMixFix_3, mark = 2, col = "grey", markcol = 1))
@ 

The default plot of the fitted model indicates that the components are
well separated. In addition component 2 has a slight overlap with
component 3, but none with component 1.
...
analyzed in \cite{mixtures:Aitkin:1999a}. The dataset is visualized in
...
model=FLXMRglmfix(family="binomial", fixed=~Drug), 
                     control=list(minprior=0.04),nrep=5, k=3, data=Mehta)
summary(mehtaMix)
@ 
...
clusters <- factor(cluster(mehtaMix), labels = paste("Cluster", 1:3)) 
...
mehtaMix <- stepFlexmix(cbind(Response, Total - Response)~Drug | Site, 
                        model=FLXMRglm(family="binomial"), k=3,
                     control=list(minprior=0.04), data=Mehta, nrep=5)
summary(mehtaMix)
@ 
...
dataset given in \cite{mixtures:Wang+Puterman:1998}. The data was
...
<<tribolium>>=
data(tribolium)
TribMix <- stepFlexmix(cbind(Remaining, Total - Remaining) ~ 1, k = 2:3,
model = FLXMRglmfix(fixed = ~ Species, family = "binomial"),
concomitant = FLXPmultinom(~ Replicate), data = tribolium)
@ 

The model which is selected as the best in
\cite{mixtures:Wang+Puterman:1998} can be estimated with:
<<wang-model>>=
modelWang <- FLXMRglmfix(fixed = ~ I(Species == "Confusum"), family =
                       "binomial") 
...
clusters <- factor(cluster(TribMixWang), labels = paste("Cluster", 1:TribMixWang@k))
...
\caption{Relative number of remaining beetles for the number of
  ...
  suggested in \cite{mixtures:Wang+Puterman:1998}.}
  \label{fig:tribolium}
\end{figure}

\cite{mixtures:Wang+Puterman:1998} also considered a model where they
omit one conspicuous observation. This model can be estimated with:
<<subset>>=
TribMixWangSub <- stepFlexmix(cbind(Remaining, Total - Remaining) ~ 1, k = 2, 
                              data = tribolium[-7,], model = modelWang,
                              concomitant = concomitantWang)
@ 

%%-----------------------------------------------------------------------  
\subsubsection{Trypanosome}
The data is used in \cite{mixtures:Follmann+Lambert:1989}. It is from
...
The model which is proposed in \cite{mixtures:Follmann+Lambert:1989}
is given by:
\begin{align*}
...
models in \cite{mixtures:Aitkin:1996}. Furthermore, it is described in
\cite{mixtures:McLachlan+Peel:2000} on page 155. It contains 32
...
<<fabric-fix>>=
data(fabricfault)
fabricMix <- stepFlexmix(Faults~1, model=FLXMRglmfix(family="poisson", fixed=~log(Length)), 
...
from zero for a signficance level of 0.01. We therefore also fit a
...
<<fabric-fix-nested>>=
fabricMix2 <- stepFlexmix(Faults~0, data=fabricfault, nrep=5, k = 2,
                          model=FLXMRglmfix(family="poisson", fixed=~log(Length),
                            nested = list(k=c(1,1), formula=list(~1,~0))))
summary(refit(fabricMix2))
fabricMix2.pred <- predict(fabricMix2, newdata = data.frame(Length = Lnew))
@ 
...
The patent data given in \cite{mixtures:Wang+Cockburn+Puterman:1998}
...
in \cite{mixtures:Wang+Cockburn+Puterman:1998} is given by:
\begin{align*}
...
FittedPat <- stepFlexmix(Patents ~ lgRD, k = 3, nrep=5, 
                         model = ModelPat, data = patent, 
...
clustering given by \code{cluster(FittedPat)}.
...
FittedPatFixed <- flexmix(Patents ~ 1, model = ModelFixed, 
                       cluster = posterior(FittedPat),
                       concomitant = FLXPmultinom(~ RDS), data = patent)             
...
plotData <- function(fitted) {
  with(patent, data.frame(Patents = c(Patents, 
                          ...
                          class = c(cluster(fitted), rep(1:3, each = nrow(newdata))),
...
plotConc <- function(fitted) {
  E <- exp(x%*%t(fitted@concomitant@coef))
...
\caption{Patent data with the fitted values of the component specific
  ...
  model in \citeauthor{mixtures:Wang+Cockburn+Puterman:1998} and with
...
The data is used in \cite{mixtures:Wang+Puterman+Cockburn:1996} and is
...
\cite{mixtures:Wang+Puterman+Cockburn:1996} is given by:
\begin{align*}
...
\cite{mixtures:Wang+Puterman+Cockburn:1996}. They propose a model
given by:
\begin{align*}
