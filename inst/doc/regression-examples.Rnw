...
<<diffModel>>=
Model2 <- FLXMRglmfix(fixed = ~ x2, nested = list(k = c(2, 1), 
  formula = c(~ x1, ~ 0)), varFix = TRUE)
...
mehtaMix <- stepFlexmix(cbind(Response, Total - Response) ~ Drug | Site, 
  model = FLXMRglm(family = "binomial"), k = 3, data = Mehta, nrep = 5,
...
<<trypanosome>>=
data("trypanosome")
TrypMix <- stepFlexmix(cbind(Dead, 1-Dead) ~ 1, k = 2, nrep = 5, 
...
<<fabric-fix-nested>>=
fabricMix2 <- stepFlexmix(Faults ~ 0, data = fabricfault, nrep = 5, 
  k = 2, model = FLXMRglmfix(family = "poisson", fixed = ~ log(Length),
...
seizMix <- stepFlexmix(Seizures ~ Treatment * log(Day), data = seizure, 
  k = 2, nrep = 5, model = FLXMRglm(family = "poisson", 
...
<<seizure>>=
seizMix3 <- stepFlexmix(Seizures ~ log(Day/27)/Treatment, data = seizure, 
  k = 2, nrep = 5, model = FLXMRglm(family = "poisson", 
  offset = log(seizure$Hours)))
summary(seizMix3)
...
<<salmonella>>=
data("salmonellaTA98")
salmonMix <- stepFlexmix(y ~ 1, data = salmonellaTA98, k = 2, nrep = 5,
